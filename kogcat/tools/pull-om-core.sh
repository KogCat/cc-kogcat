...
# packaged.
#
# Release asset: `om-core-bin-<target>.<ext>` — the onedir bundle packed
# as `tar.xz` (macOS/Linux) or `zip` (Windows), members at the archive
# root. This script downloads it and extracts into bin/om-core-<target>/.
#
# Usage:
...
#                   source of truth — the publishing repo is baked into this
#                   URL at publish time, so pulling tracks publishing
...
for target in "${TARGETS[@]}"; do
    case "$target" in
        *windows*) ext=".exe"; asset_ext="zip" ;;
        *) ext=""; asset_ext="tar.xz" ;;
    esac
    asset="om-core-bin-${target}.${asset_ext}"
    ...
    case "$asset_ext" in
        zip)
            if command -v unzip >/dev/null 2>&1; then
                ...
                python3 -m zipfile -e "$archive" "$target_dir"
            fi
            ;;
