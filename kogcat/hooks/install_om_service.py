...
def _resolve_bin() -> Path | None:
    """Resolve the binary the supervisor should register — the version
    SessionStart actually fetched (channel-first), not the bundled floor.

    Spec 19: register the canonical cache path, NOT whatever OM_CORE_BIN
    points at. The rolling channel can be ahead of the plugin's bundled
    manifest, so the bundled version's bin may never land on disk (the
    fetcher pulled the channel version instead). Prefer the version
    `expected-release.json` records (what bootstrap resolved + fetched this
    session); fall back to the bundled manifest floor. Return the first
    candidate whose cache bin exists, else None (fetch still in flight →
    ...
    try:
        ...
        if expected and expected.get("version"):
            candidates.append(str(expected["version"]))
        try:
            bundled = om_core_paths.read_manifest(plugin_root, target).get("version")
...
def _is_already_installed(bin_path: Path) -> bool:
    ...
    try:
        ...
        return False
    # Windows logon-autostart (run_key / schtasks ONLOGON) registers but does
    # NOT start the daemon in the install session. Require running too, so a
    # "registered but down" host re-runs install-service, which starts it now.
    return bool(status.get("installed")) and bool(status.get("running"))
...
def main() -> int:
    ...
    if cfg_dir_str:
        ...
        from pathlib import Path as _P
        cfg_dir = _P(os.environ.get("LOCALAPPDATA", _P.home() / "AppData" / "Local")) / "om"
    else:
        from pathlib import Path as _P
